...
* `processCall(`<string>`) `[WinBatch Options]` : string (exitcode)` //since 4.11.6.1 [W/L/M] +
Startet das Programm <string> als Prozess und liefert den Exitcode zurück. +
[source,winst]
----
set $exitcode$ = processCall('setup.exe /S') /SysNative
----
Ist eine Abkürzung für den Ausdruck:
[source,winst]
----
...
setup.exe /S
----
Tatsächlich basiert `processCall` intern auf den selben Routinen wie `winbatch` und verarbeitet daher auch xref:opsi-winst-winbatch-params[dieselben Modifier], wie zum Beispiel:
...
* `/showoutput` //seit <EXPERIMENTAL> [W/L/M] +
Wenn diese Option gesetzt ist, wird die Ausgabe der Prozesse, die in der Sektion gestartet werden, bei der Ausführung in einem Fenster angezeigt.

* `/RunElevated`  [W] +
Started dem Prozess mit einem höheren Security Token (d.h. mit höheren Rechten). Dieser Modifier hat folgende Einschränkungen: +
...
Zeigt die Ausgabe des gestarten Skripts bei der Ausführung in einem zusätzlichen Fenster an.

* `/RunElevated` // seit <EXPERIMENTAL> [W] +
Startet das Skript mit einem höheren Security Token, was vor allem für bestimmte Operationen mit `powershell.exe`, die mehr Rechte brauchen, interessant ist. Dieser Modus funktioniert nur im `opsi-service`-Kontext und erlaubt keinen Zugriff auf das Netzwerk.

* `/RunAsLoggedOnUser` // seit <EXPERIMENTAL> [W] +
Nur im Kontext eines `userLoginScript`s verfügbar. Startet das Skript als der Benutzer, der sich gerade einloggt.
