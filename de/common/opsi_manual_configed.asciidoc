...
Außerdem dürfen nur jene Benutzer überhaupt Befehle auf gewissen Servern ausführen oder die Konsole verwenden, die sich mittels SSH auf diesem Server verbinden dürfen oder, die den entsprechenden Server im Userrole-Config "user.{}.privilege.host.depotaccess.depots" freigegeben haben. Falls Userroles nicht freigeschaltet ist, darf man sich configed-seitig an alle für opsi bekannten Servern verbinden. +
...
Neben den Configs für diverse Menüaktivierungen wurde ein weiterer eingeführt : "configed.workbench.default". Dieser ist standardmäßig auf "/home/opsiproducts/" gesetzt. Wird ein anderes Produkt-Bau-Verzeichnis verwendet, kann es an dieser Stelle hinterlegt werden. Es wird lediglich zur schnelleren Navigation z.B. bei der opsi-makeproductfile Oberfläche verwendet.
