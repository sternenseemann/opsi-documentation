...
Eintrag des Clienst in der Datenbank geben. Wenn es diesen Werte-Eintrag gibt, bleibt er auch erhalten, wenn sich der
Server-Default ändert.

- Daher gibt es für den Client-Properties via Kontextmenü die beiden Optionen "Spezifischen Client-Wert entfernen" 
(d.h., der Clientwert ist immer der gerade aktuelle Default-Werte des Servers) und  "Setze den jetzigen Default-Wert
als (dauerhafen) Wert für den Client.

- Wenn ein spezifischer Client-Wert sich vom Server-Default unterscheidet, so ist dieser fett dargestellt. 

- Es kann Konfigurationsobjekte geben, für die Client-Werte erzeugt und bearbeitet
werden können, die jedoch keine Bedeutung haben. Diese Properties sind allerdings in der Regel ausgeblendet und treten daher 
