...
The allowed server are (without userroles feature:) all known server for opsi -- otherwise all allowed depots should be define in the config "user.{}.privilege.host.depotaccess.depots" are allowed.
...
To define the product build directory (default: "/home/opsiproducts/"), which is needed e.g. in the opsi-makeproductfile gui, a new config was created "configed.workbench.default". 
