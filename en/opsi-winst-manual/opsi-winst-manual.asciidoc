...
setup.exe /S
----
In fact `processCall` is a winbatch call internally, so all the xref:opsi-winst-winbatch-params[winbatch modifiers] are also allowed for `processCall` like for example:
...
* `/RunElevated`  [W] +
Starts a process using a security token with elevated privileges. This modifier has the following restrictions: +
...
* `/showoutput` //since EXPERIMENTAL [W/L/M] +
Show the output of the started process(es) in a separate window during execution.
